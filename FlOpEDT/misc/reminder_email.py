...
def concerned_tutors(week_list, department):
    ...
    for tutor in Tutor.objects.filter(departments=department):
        if Course.objects.filter(
            week__in=week_list, tutor=tutor, type__department=department
        ):
...
def send_reminder_email(
    ...
    if len(week_list) == 1:
        ...
        week_text = (
            f"des semaines {', '.join(week_list[i].nb for i in range(len(week_list)-1))}"
            + f" et {week_list[-1].nb}"
        )
    ...
    if url is not None:
        msg += (
            f"Merci de mettre à jour vos disponibilités sur le site"
    ...
    msg += (
        "Merci, et comme d'habitude n'hésitez pas à nous écrire pour toute remarque/question.\n"
        "Les gestionnaires de l'EDT\n\n"
    )
    msg += (
        "NB: Ceci est un message automatique envoyé par flop!EDT, gestionnaire d'emploi du temps "
    ...
    if tutors is None:
        ...
        recipients = [
            str(tutor.username) + " <" + str(tutor.email) + ">" for tutor in tutors
        ]
    email = EmailMessage(subject, msg, to=[sender_email], bcc=recipients)
