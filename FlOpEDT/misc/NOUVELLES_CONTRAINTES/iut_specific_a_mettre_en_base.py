...
from base.models import CourseType, Department, TrainingProgramme
...
def add_common_constraints():
    ...
    for department in Department.objects.filter(abbrev__in=["INFO", "RT", "GIM", "CS"]):
        ...
        M, created = MinGroupsHalfDays.objects.get_or_create(
        ...
        if pas_plus_de_2_exams_par_jour:
            L, created = LimitGroupsTimePerPeriod.objects.get_or_create(
                max_hours=3,
                course_type=DS,
        ...
        if un_amphi_par_matiere_par_jour:
            L, created = LimitModulesTimePerPeriod.objects.get_or_create(
                max_hours=2,
                course_type=CM,
                period=LimitTimePerPeriod.FULL_DAY,
                department=department,
...
def add_info_constraints():
    ...
    M, created = MinModulesHalfDays.objects.get_or_create(
        weight=max_weight, department=department
    )
    M.save()
    if created:
    ...
    M, created = MinTutorsHalfDays.objects.get_or_create(
        join2courses=True, weight=max_weight, department=department
    )
    M.save()
    if created:
