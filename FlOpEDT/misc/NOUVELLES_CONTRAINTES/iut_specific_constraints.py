...
def add_iut_blagnac_specials(ttmodel):
    ...
    pas_jeudi_PM = ttmodel.data.courses.exclude(groups__train_prog__abbrev__in=['APSIO', 'RT2A'])
    ...
    for sport in sports:
        ...
        if not cours_de_sport:
            continue
        ttmodel.add_constraint(ttmodel.sum(ttmodel.scheduled[sl, c]
                                           for sl in set(slot for slot in ttmodel.data.courses_slots
    ...
    if promos1.exists():
        if not ttmodel.department.abbrev == 'RT':
            ttmodel.add_constraint(ttmodel.sum(ttmodel.scheduled[sl, c]
                                               ...
                                               for c in set(ttmodel.data.courses.filter(group__train_prog__in=promos1)
            ...
            for sl in ttmodel.data.courses_slots:
                if sl.end_time > 17 * 60 + 15 and sl.day in [Day.MONDAY, Day.TUESDAY]:
                    ttmodel.add_to_slot_cost(sl, 10000 * ttmodel.sum(ttmodel.scheduled[sl, c]
                                                                     for c in set(ttmodel.data.courses.
                                                                                  filter(groups__train_prog__in=promos1)
                                                                                  .exclude(module__abbrev='SC'))
                                                                     & ttmodel.data.compatible_courses[sl]))

    if 41 in ttmodel.weeks and ttmodel.department.abbrev in ['RT', 'INFO']:
        C = ttmodel.data.courses.filter(groups__train_prog__abbrev__in=['INFO2', 'RT2', 'RT2A', 'APSIO']) \
            .exclude(module__abbrev='CONF')
        mardi_11h = slots_filter(ttmodel.data.courses_slots, weekday=Day.TUESDAY, start_time=11 * 60, week=41).pop()
        ttmodel.add_constraint(ttmodel.sum(ttmodel.scheduled[sl, c]
                                           for sl in slots_filter(ttmodel.data.courses_slots, simultaneous_to=mardi_11h)
...
def add_iut_blagnac_info(ttmodel):
    ...
    if IC in ttmodel.data.instructors:
        for week in {41, 42}:
            if week in ttmodel.weeks:
                ttmodel.add_constraint(ttmodel.sum(ttmodel.IBD[IC, d] for d in
                                                   days_filter(ttmodel.data.days, week=week)),
                                       '>=',
                                        4,
        ...
        for d in ttmodel.data.days:
            sl15 = slots_filter(ttmodel.data.availability_slots, day=d, start_time=15 * 60 + 45).pop()
            sl17 = slots_filter(ttmodel.data.availability_slots, day=d, start_time=17 * 60 + 15).pop()
    ...
    if PDU in ttmodel.data.instructors:
        for day in ttmodel.data.days:
            sl8 = slots_filter(ttmodel.data.availability_slots, day=day, start_time=8 * 60).pop()
            sl11 = slots_filter(ttmodel.data.availability_slots, day=day, start_time=11 * 60).pop()
            sl14 = slots_filter(ttmodel.data.availability_slots, day=day, start_time=14 * 60 + 15).pop()
    ...
    if 40 in ttmodel.weeks and ttmodel.year == 2019:
        ...
        for sl in slots_filter(ttmodel.data.courses_slots, weekday=Day.FRIDAY, apm=Time.PM, week=40):
    ...
    if len(cms_peda) == 2:
        ...
        for sl in ttmodel.data.compatible_slots[c1]:
            if sl.start_time > 15 * 60 or sl.start_time < 9 * 60:
                ttmodel.add_constraint(ttmodel.scheduled[(sl, c1)] +
                                       ttmodel.scheduled[(sl, c2)], '==', 0,
        ...
        for week in [2, 5, 7, 14]:
            if week in ttmodel.data.weeks:
                for day in days_filter(ttmodel.data.days, week=week, day=Day.THURSDAY):
                    for i in set(tutor for tutor in ttmodel.data.instructors if tutor.status == Tutor.FULL_STAFF):
    ...
    if prof_cdu in ttmodel.data.instructors:
        for week in ttmodel.weeks:
            cours_cdu = set(c for c in ttmodel.data.courses_for_tutor[prof_cdu] if c.week == week)
            if len(cours_cdu) >= 6:
                cdu6 = {}
                ...
                for rang, jour in enumerate([Day.MONDAY, Day.TUESDAY, Day.WEDNESDAY]):
                    ...
                    slots = slots_filter(ttmodel.data.courses_slots, weekday=jour, week=week)
                    jour_sum = ttmodel.sum(
                        ttmodel.scheduled[sl, c] for sl in slots for c in cours_cdu & ttmodel.data.compatible_courses[sl])
                    ...
                    if len(cours_cdu) == 7:
                        suivant_8h = slots_filter(ttmodel.data.courses_slots, weekday=jours[rang + 1], start_time=8 * 60,
                                                  week=week).pop()
                        ttmodel.add_constraint(
                            cdu6[jour] -
                            ttmodel.sum(ttmodel.scheduled[suivant_8h, c] for c in
                                        cours_cdu & ttmodel.data.compatible_courses[suivant_8h]),
                            '==',
                            0,
    ...
    if 3 in ttmodel.weeks and ttmodel.year == 2020:
        ...
        for sl in slots_filter(ttmodel.data.courses_slots, week=3):
            ...
            ttmodel.add_constraint(10 * used_slot[sl] -
                                   ttmodel.sum(ttmodel.scheduled[sl, c] for c in C & ttmodel.data.compatible_courses[sl]),
                                   '>=', 0,
                                   Constraint(constraint_type=ConstraintType.SEMAINE_3_EXAM_ISI))
        ttmodel.add_constraint(ttmodel.sum(used_slot[sl] for sl in ttmodel.data.courses_slots), '<=', 2,
                               Constraint(constraint_type=ConstraintType.SEMAINE_3_EXAM_ISI))
    ...
    if C:
        ...
        for sl in slots_filter(ttmodel.data.courses_slots):
            ...
            ttmodel.add_constraint(20 * used_slot[sl] -
                                   ...
                                   '>=', 0,
                                   Constraint(constraint_type=ConstraintType.SEMAINE_3_EXAM_ISI))
        ttmodel.add_constraint(ttmodel.sum(used_slot[sl] for sl in ttmodel.data.courses_slots), '<=', 3,
                               Constraint(constraint_type=ConstraintType.SEMAINE_3_EXAM_ISI))
    ...
    C = set(ttmodel.data.courses.filter(module__abbrev='AMN', type__name='TD', tutor__username__in=['PSE','---']))
    if C:
        n = len(C)
        used_slot = {}
        for sl in slots_filter(ttmodel.data.courses_slots):
            ...
            ttmodel.add_constraint(20 * used_slot[sl] -
                                   ...
                                   '>=', 0,
                                   Constraint(constraint_type=ConstraintType.SEMAINE_3_EXAM_ISI))
        ttmodel.add_constraint(ttmodel.sum(used_slot[sl] for sl in ttmodel.data.courses_slots), '<=', n/2,
                               Constraint(constraint_type=ConstraintType.SEMAINE_3_EXAM_ISI))
    ...
    if 4 in ttmodel.weeks and ttmodel.year == 2019:
        ...
        for day in [Day.THURSDAY, Day.FRIDAY]:
            ttmodel.add_constraint(ttmodel.sum(ttmodel.scheduled[sl, c]
                                               for c in C
                                               for sl in slots_filter(ttmodel.data.courses_slots, weekday=day, week=4)
    ...
    if 23 in ttmodel.weeks or 24 in ttmodel.weeks:
        pas_AM = ttmodel.data.courses.filter(groups__train_prog__abbrev__in=['INFO1'], week__in={23,24})
        # if 19 not in ttmodel.weeks:
        if True:
            ttmodel.add_constraint(ttmodel.sum(ttmodel.scheduled[(sl, c)]
                                               for c in pas_AM
    ...
    if 6 in ttmodel.weeks and ttmodel.year == 2019:
        ...
        ttmodel.add_constraint(ttmodel.sum(ttmodel.scheduled[(sl, c)]
                                           for i in ttmodel.data.instructors if i.status == Tutor.FULL_STAFF
                                           for c in ttmodel.data.courses_for_tutor[i]
                                           for sl in slots_filter(ttmodel.data.courses_slots, weekday=Day.FRIDAY, apm=Time.PM,
                                                                  week=6)
                                           & ttmodel.data.compatible_slots[c]),
                               '==',
                               0,
    ...
    if 9 in ttmodel.weeks and ttmodel.year == 2020:
        for c in ttmodel.data.courses.filter(module__abbrev='FLOP'):
            ...
            slots = slots_filter(ttmodel.data.courses_slots, weekday=sc.day, start_time=sc.start_time)
            ttmodel.add_constraint(
                ttmodel.sum(ttmodel.located[(slot, c, sc.room)] for slot in slots & ttmodel.data.compatible_slots[c]),
                '==',
                1,
...
def add_iut_blagnac_rt(ttmodel):
    ...
    for i in range(1, 5):
        ...
        if a > 0:
            ...
            for sl in ttmodel.data.compatible_slots[C_A[0]]:
                if contrainte:
                    ttmodel.add_constraint(
                        ttmodel.sum(ttmodel.scheduled[sl, c] for c in C_A) - ttmodel.sum(ttmodel.scheduled[sl, c] for c in C_EC),
                        relation,
                        0, Constraint())
                else:
                    if a < ec:
                        ttmodel.add_to_slot_cost(sl, ttmodel.sum(ttmodel.scheduled[sl, c] for c in C_EC) - ttmodel.sum(
                            ttmodel.scheduled[sl, c] for c in C_A))
                    else:
                        ttmodel.add_to_slot_cost(sl,
                                                 ttmodel.sum(ttmodel.scheduled[sl, c] for c in C_A) - ttmodel.sum(
                                                     ttmodel.scheduled[sl, c] for c in C_EC))
    ...
    for week in ttmodel.weeks:
        for m in ttmodel.data.modules.filter(period__department=RT):
            ...
            courses = set(courses)
            tp240unique = {}
            for sl in slots_filter(ttmodel.data.courses_slots, course_type=ct, week=week):
            ...
            if not assez_de_permanents:
                continue
            for sl in slots_filter(ttmodel.data.courses_slots, course_type=ct, week=week):
                vacataires = ttmodel.sum(ttmodel.scheduled[sl, c]
                                         for c in courses & ttmodel.data.compatible_courses[sl]
                                         if c.tutor.status == 'ss')
                permanents = ttmodel.sum(ttmodel.scheduled[sl, c]
                                         for c in courses & ttmodel.data.compatible_courses[sl]
        ...
        for g in ttmodel.data.groups.filter(type__name='TD'):
            for d in ttmodel.data.days:
                for apm in [Time.AM, Time.PM]:
                    ttmodel.add_constraint(
                        ttmodel.sum(ttmodel.scheduled[sl, c]
                                    for c in ttmodel.data.courses.filter(module=m, groups=g, type__name='TD')
                                    for sl in slots_filter(ttmodel.data.courses_slots, day=d, apm=apm)
                                    & ttmodel.data.compatible_slots[c]),
                        '<=',
                        2,
    ...
    for t in ttmodel.data.instructors:
        if t.status == 'ss':
            for week in ttmodel.weeks:
                heures = sum(c.type.duration for c in ttmodel.data.courses_for_tutor[t] if c.week == week) / 60
    ...
    for t in ttmodel.data.instructors:
        if t.username in ['PCOU', 'LAND']:
            for week in ttmodel.weeks:
                heures = sum(c.type.duration for c in ttmodel.data.courses_for_tutor[t] if c.week == week) / 60
                dispos = set(a for a in ttmodel.data.availabilities[t][week] if a.value >= 1)
                ...
                for d in days_filter(ttmodel.data.days, week=week):
                    for apm in [Time.AM, Time.PM]:
                        ttmodel.add_constraint(
                            ttmodel.sum(ttmodel.scheduled[sl, c] * c.type.duration
                                        for sl in slots_filter(ttmodel.data.courses_slots, day=d, apm=apm)
    ...
    if 36 in ttmodel.weeks:
        ttmodel.add_constraint(ttmodel.sum(ttmodel.scheduled[sl, c]
                                           for sl in slots_filter(ttmodel.data.courses_slots, start_time=8 * 60,
...
def add_iut_blagnac_gim(ttmodel):
    ...
    for bg in ttmodel.data.basic_groups.filter(train_prog__abbrev='GIM1').exclude(name__in=['1AA', '1AM']):
        for sl1 in ttmodel.data.courses_slots:
            for sl2 in slots_filter(ttmodel.data.courses_slots, simultaneous_to=sl1) - {sl1}:
                ...
                ttmodel.add_constraint(ttmodel.sum(ttmodel.scheduled[(sl1, c1)]
                                                   ...
                                                   & ttmodel.data.compatible_courses[sl1]) * 0.5 +
    ...
    for m in ttmodel.data.modules:
        ...
        if m.abbrev in ['OMM4-STA4', 'MTS4-STA3']:
            G1 = [ttmodel.data.groups.get(name='TPA2', train_prog__abbrev='GIM2'),
                  ttmodel.data.groups.get(name='TPB2', train_prog__abbrev='GIM2')]
            G2 = [ttmodel.data.groups.get(name='TPB1', train_prog__abbrev='GIM2'),
                  ttmodel.data.groups.get(name='TPC1', train_prog__abbrev='GIM2')]
        for nb in range(len(G1)):
            C1 = list(c for c in ttmodel.data.courses_for_group[G1[nb]] if c.module == m)
            if C1:
                C2 = list(c for c in ttmodel.data.courses_for_group[G2[nb]] if c.module == m)
                if len(C1) != len(C2):
                ...
                for i in range(len(C1)):
                    ...
                    c2 = C2[i]
                    for sl in ttmodel.data.compatible_slots[c1]:
                        ttmodel.add_constraint(ttmodel.scheduled[sl, c1] - ttmodel.scheduled[sl, c2], '==', 0,
    ...
    for m in ttmodel.data.modules.filter(period__department=GIM).exclude(abbrev__in=['OMM4-STA4', 'MTS4-STA3']):
        for bg in ttmodel.data.basic_groups:
            for d in ttmodel.data.days:
                ttmodel.add_constraint(
                    ttmodel.sum(ttmodel.scheduled[sl, c]
                                for c in ttmodel.data.courses_for_basic_group[bg] if c.module == m
    ...
    if 2 in ttmodel.weeks:
        ...
        ttmodel.add_constraint(ttmodel.sum(ttmodel.scheduled[sl, c] for c in C
                                           for sl in ttmodel.data.compatible_slots[c] if sl.day.day != Day.FRIDAY),
                               '==',
                               0,
    ...
    if 6 in ttmodel.weeks:
        ...
        ttmodel.add_constraint(ttmodel.sum(ttmodel.scheduled[sl, c] for c in CDBE
                                           for sl in ttmodel.data.compatible_slots[c] if
        ...
        ttmodel.add_constraint(ttmodel.sum(ttmodel.scheduled[sl, c] for c in CPR
                                           for sl in ttmodel.data.compatible_slots[c] if
                                           sl.day.day != Day.MONDAY
                                           or sl.apm != Time.PM
                                           ),
        ...
        ttmodel.add_constraint(ttmodel.sum(ttmodel.scheduled[sl, c] for c in CMPH
                                           for sl in ttmodel.data.compatible_slots[c] if
        ...
        ttmodel.add_constraint(ttmodel.sum(ttmodel.scheduled[sl, c] for c in CEPI
                                           for sl in ttmodel.data.compatible_slots[c] if
...
def add_iut_blagnac_cs(ttmodel):
    ...
    for week in ttmodel.weeks:
        FV = ttmodel.data.courses.filter(module__abbrev='TECH', tutor__username='FV', week=week)
        NVIG = ttmodel.data.courses.filter(module__abbrev='TECH', tutor__username='NVIG', week=week)
        if FV.count() == 2 or NVIG.count() == 2:
            ttmodel.add_constraint(ttmodel.sum(ttmodel.scheduled[sl, c] for c in FV | NVIG
                                               for sl in ttmodel.data.compatible_slots[c]
                                               - slots_filter(ttmodel.data.compatible_slots[c],
    ...
    for week in ttmodel.weeks:
        ...
        M.save()
        for i in ttmodel.data.instructors:
            if sum(c.type.duration / 60 for c in ttmodel.data.courses_for_tutor[i] if c.week == week) <= 180:
                M.tutors.add(i)
        M.save()
