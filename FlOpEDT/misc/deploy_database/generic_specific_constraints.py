...
def add_iut_blagnac_lp():
    ...
    R = ReasonableDays(train_prog=lp, department=info, weight=max_weight)
    R.save()

    # Force that only 3 courses are the same HD, and only on 2 HD if 4 courses
    for module in Module.objects.filter(train_prog=lp):
        M = MinHalfDays(
            module=module, department=info, join2courses=True, weight=max_weight
        )
        M.save()
...
def add_iut_blagnac_other():
    ...
    for i in Tutor.objects.all():
        R = ReasonableDays(tutor=i, department=info, weight=max_weight)
        R.save()
    ...
    for i in Tutor.objects.filter(
        ...
        M = MinHalfDays(tutor=i, department=info, join2courses=True, weight=max_weight)
        M.save()
    ...
    if cms_peda.count() >= 2:
        S = SimultaneousCourses(
            course1=cms_peda[0], course2=cms_peda[1], department=info
        )
        S.save()
