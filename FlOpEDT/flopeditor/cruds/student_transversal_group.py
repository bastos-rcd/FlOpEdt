...
def read(department):
    ...
    parallel_groups_choices = [
        g.name for g in transversal_groups
    ]  # STAGE Empêcher de mettre le groupe en parallèle avec lui même (je ne sais pas si c'est ici)
    ...
    for group in transversal_groups:
        ...
        values.append(
            (
                ...
                conflicting_groups,
                parallel_groups,
                group.type.name if group.type is not None else None,
                group.size,
            )
        )
    ...
    return JsonResponse(
        {
            "columns": [
                ...
                },
                {
                    ...
                    "type": "select-chips",
                    "options": {"values": conflicting_groups_choices},
                },
                {
                    "name": "Parallèle à...",
                    "type": "select-chips",
                    "options": {"values": parallel_groups_choices},
                },
                {
                    "name": "Nature",
                    "type": "select",
    ...
    for i in range(len(entries["new_values"])):
        ...
        try:
            ...
            for group_name in entries["new_values"][i][2]:
                new_conflicting_groups.append(
                    StructuralGroup.objects.get(name=group_name, train_prog=train)
                )
            new_parallel_groups = []
            for group_name in entries["new_values"][i][3]:
                new_parallel_groups.append(
                    TransversalGroup.objects.get(name=group_name, train_prog=train)
                )

            if validate_student_transversal_groups_values(
                ...
                if (
                    ...
                    group = TransversalGroup.objects.create(
                        name=new_name, size=size, train_prog=train, type=gtype
                    )
...
def update(entries, department):
    ...
    for i in range(len(entries["old_values"])):
        if not validate_student_transversal_groups_values(
            entries["new_values"][i], entries
        ):
            return entries
        ...
        try:
            ...
            if (new_name != old_name or new_tp_abbrev != old_tp_abbrev) and (
                TransversalGroup.objects.filter(
                    name=new_name, train_prog=new_train
                ).exists()
                or StructuralGroup.objects.filter(
                    name=new_name, train_prog=new_train
                ).exists()
