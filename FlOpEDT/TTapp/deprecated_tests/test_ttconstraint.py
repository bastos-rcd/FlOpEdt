...
class TimetableConstraintTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.c_basic = LimitTimePerPeriod.objects.create(
            limit=1, type=self.TD, department=self.info
        )

        self.c_2018 = LimitTimePerPeriod.objects.create(
            train_prog=self.dut1, year=2018, limit=1, type=self.TD, department=self.info
        )
        self.c_2018_39 = LimitTimePerPeriod.objects.create(
            train_prog=self.dut1,
            year=2018,
            week=39,
            limit=0,
            type=self.TD,
            department=self.info,
        )
        self.c_2018_39_without_tp = LimitTimePerPeriod.objects.create(
            year=2018, week=39, limit=0, type=self.TD, department=self.info
        )

        self.c_tp2 = LimitTimePerPeriod.objects.create(
            train_prog=self.dut2, limit=0, type=self.TD, department=self.info
        )
        self.c_2019_1 = LimitTimePerPeriod.objects.create(
            train_prog=self.dut2,
            year=2019,
            week=1,
            limit=0,
            type=self.TD,
    ...
    def test_limit_register_expression_without_tutors(self, register_expression):
        ...
        calls = [
            call(ttmodel, period_by_day, 1.0),
        ]

        constraint = LimitTimePerPeriod.objects.create(
            limit=1, type=self.TD, department=self.info
        )
    ...
    def test_limit_register_expression_with_tutors(self, register_expression):
        ...
        for tutor in Tutor.objects.filter(username__in=["AB", "AJ", "CDU", "FMA"]):
            calls.append(call(ttmodel, period_by_day, 1.0, tutor=tutor))
...
class CustomConstraintTestCase(TestCase):
    ...
    @patch(
        "TTapp.models.CustomConstraint.class_name",
        "MyFlOp.custom_constraints.FirstCustomConstraint",
    )
    def test_get_constraint(self, target_class):
        custom_constraint = CustomConstraint()
        constraint = custom_constraint.get_constraint(
            "MyFlOp.custom_constraints.FirstCustomConstraint"
        )
    ...
    def test_enrich_model(self, constraint_class):
        custom_constraint = CustomConstraint.objects.create(
            class_name="MyFlOp.custom_constraints.FirstCustomConstraint"
        )
