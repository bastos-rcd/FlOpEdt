...
class NoVisio(TimetableConstraint):
    ...
    groups = models.ManyToManyField(
        "base.StructuralGroup", blank=True, related_name="no_visio"
    )
    course_types = models.ManyToManyField(
        "base.CourseType", blank=True, related_name="no_visio"
    )
    ...
    def enrich_ttmodel(self, ttmodel, period, ponderation=1000000):
        if hasattr(self.department, "mode"):
        ...
        for group in considered_groups:
            ...
            if self.weight is None:
                ...
                considered_group_courses = (
                    ttmodel.data.courses_for_basic_group[group]
                    - ttmodel.data.visio_courses
                )

            if self.course_types.exists():
                considered_group_courses = set(
                    c
            ...
            if self.weight is None:
                ...
                ttmodel.add_to_group_cost(
                    group, self.local_weight() * ponderation * relevant_sum
                )
...
class VisioOnly(TimetableConstraint):
    ...
    groups = models.ManyToManyField(
        "base.StructuralGroup", blank=True, related_name="visio_only"
    )
    course_types = models.ManyToManyField(
        "base.CourseType", blank=True, related_name="visio_only"
    )
    modules = models.ManyToManyField(
        "base.Module", blank=True, related_name="visio_only"
    )
    ...
    def enrich_ttmodel(self, ttmodel, period, ponderation=1000):
        ...
        for d in days:
            for apm in [Time.AM, Time.PM]:
                physically_presents_groups_number = ttmodel.sum(
                    ttmodel.physical_presence[g][d, apm]
                    for g in ttmodel.data.basic_groups
                )
                is_over_bound = ttmodel.add_floor(
                    physically_presents_groups_number,
                ...
                if self.weight is None:
                    ttmodel.add_constraint(
                        ...
                        0,
                        Constraint(
                            constraint_type=ConstraintType.VISIO_LIMIT_GROUP_PRESENCE
                        ),
                    )
                else:
                    ttmodel.add_to_generic_cost(
                        is_over_bound * self.local_weight() * ponderation, period=period
                    )
...
class BoundPhysicalPresenceHalfDays(TimetableConstraint):
    ...
    nb_max = models.PositiveSmallIntegerField(
        validators=[MinValueValidator(0), MaxValueValidator(14)], default=14
    )
    nb_min = models.PositiveSmallIntegerField(
        validators=[MinValueValidator(0), MaxValueValidator(14)], default=0
    )
    groups = models.ManyToManyField(
        "base.StructuralGroup",
        blank=True,
        related_name="bound_physical_presence_half_days",
    )
    ...
    def enrich_ttmodel(self, ttmodel, period, ponderation=1):
        ...
        for g in considered_groups:
            physical_presence_half_days_number[g] = ttmodel.sum(
                ttmodel.physical_presence[g][d, apm]
        ...
        if self.weight is None:
            for g in considered_groups:
                ...
                ttmodel.add_constraint(
                    ...
                    self.nb_min,
                    Constraint(
                        constraint_type=ConstraintType.MIN_PHYSICAL_HALF_DAYS,
                        groups=g,
                        periods=period,
                    ),
                )
                ...
                ttmodel.add_constraint(
                    ...
                    self.nb_max,
                    Constraint(
                        constraint_type=ConstraintType.MAX_PHYSICAL_HALF_DAYS,
                        groups=g,
                        periods=period,
                    ),
                )
...
class Curfew(TimetableConstraint):
    ...
    def enrich_ttmodel(self, ttmodel, period, ponderation=2):
        ...
        if self.weight is None:
            ttmodel.add_constraint(
                relevant_sum, "==", 0, Constraint(constraint_type=ConstraintType.CURFEW)
            )
        else:
            ttmodel.add_to_generic_cost(
