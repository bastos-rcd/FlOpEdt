...
class BreakingNews(models.Model):
    ...
    x_beg = models.FloatField(default=2.0, blank=True)
    x_end = models.FloatField(default=3.0, blank=True)
    y = models.PositiveSmallIntegerField(null=True, default=None, blank=True)
...
class TutorDisplay(models.Model):
    tutor = models.OneToOneField(
        "people.Tutor", related_name="display", on_delete=models.CASCADE
    )
...
class ModuleDisplay(models.Model):
    module = models.OneToOneField(
        "base.Module", related_name="display", on_delete=models.CASCADE
    )
...
class TrainingProgrammeDisplay(models.Model):
    training_programme = models.OneToOneField(
        "base.TrainingProgramme", related_name="display", on_delete=models.CASCADE
    )
    ...
    def __str__(self):
        return (
            f"{self.training_programme} : Row {self.row} ; " + f"Name {self.short_name}"
        )


class GroupDisplay(models.Model):
    group = models.OneToOneField(
        "base.StructuralGroup", related_name="display", on_delete=models.CASCADE
    )
    ...
    def __str__(self):
        return (
            f"{self.group} -> BH: {self.button_height} ; " + f"BTXT: {self.button_txt}"
        )
