...
def favicon(req, fav, **kwargs):
    return RedirectView.as_view(
        url=staticfiles_storage.url("base/img/favicons/" + fav), permanent=False
    )(req)
...
def index(req):
    ...
    def redirect_to_edt(department):
        reverse_url = reverse(
            "base:edt-blank", kwargs={"department": department.abbrev}
        )
        return reverse_url
...
def stype(req, *args, **kwargs):
    ...
    if req.method == "GET":
        ...
        if "apply" in list(req.POST.keys()):
            ...
            date_fin = {"week": req.POST["week_end"], "year": req.POST["year_end"]}
...
def room_preference(req, department, tutor=None):
    roomtypes = RoomType.objects.filter(department=req.department).prefetch_related(
        "members"
    )
    ...
    for rt in base_pref:
        ...
        while base_pref[rt]:
            ...
            if not best:
                ...
                base_pref[rt] = [p for p in base_pref[rt] if p["better"] not in best]
                for rg in best:
                    ranked_rg.add(rg)
                    pref[rt][rg] = rank
                rank += 1
    ...
    pref_js = {
        rt.id: {rg.id: pref[rt][rg] for rg in rt.members.all()} for rt in roomtypes
    }

    return render(
        req,
        "base/room_preference.html",
        {
            "user": req.user,
            "roomtypes": rt_dict,
            "roomgroups": rg_dict,
...
def fetch_user_notifications_pref(req, username=None, *args, **kwargs):
    ...
    return JsonResponse(
        {"nb_weeks": queries.get_notification_preference(user)}, safe=False
    )
...
def decale(req, **kwargs):
    ...
    for p in Tutor.objects.filter(departments=department).order_by("username"):
        liste_profs.append(p.username)

    return TemplateResponse(
        req,
        "base/show-decale.html",
        {
            "all_weeks": week_list(),
            "week_init": week_init,
            "year_init": year_init,
...
def all_modules_with_desc(req, **kwargs):
    return TemplateResponse(
        req,
        "base/modules.html",
        {"is_tutor": req.user.is_tutor if req.user.is_authenticated else False},
    )
...
def fetch_all_tutors(req, **kwargs):
    ...
    tutor_list = [
        t.user.username
        for t in UserDepartmentSettings.objects.filter(
            department=req.department, user__is_tutor=True
        )
    ]
...
def fetch_decale(req, **kwargs):
    ...
    for c in course:
        try:
            cp = ScheduledCourse.objects.get(course=c, work_copy=0)
            day = cp.day
            time = cp.start_time
        except ObjectDoesNotExist:
            day = ""
    ...
    course = (
        filt_p(filt_g(filt_sa(department, week, year), group), prof)
    ...
    course = (
        filt_g(filt_sa(department, week, year), group)
    ...
    course = filt_p(filt_m(filt_sa(department, week, year), module), prof).distinct(
        "groups"
    )
    groups = set()
    for c in course:
    ...
    groups = [{"training_programme": g[0], "group": g[1]} for g in groups]
...
def fetch_flat_rooms(req, **kwargs):
    ...
    return JsonResponse(
        [
            {"room": room.name}
            for room in queries.get_rooms(req.department.abbrev, basic=True)
        ],
        safe=False,
    )
...
def fetch_extra_sched(req, year, week, **kwargs):
    ...
    for scheduled in ScheduledCourse.objects.filter(
        ...
        course__year=year,
        work_copy=0,
        course__room_type__department=req.department,
    ).distinct("course__tutor"):
        tutor = scheduled.course.tutor
        if UserDepartmentSettings.objects.filter(user=tutor).count() > 1:
            tutors.append(tutor)

    dataset = MultiDepartmentTutorResource().export(
        ScheduledCourse.objects.filter(
            course__week=week,
            course__year=year,
            work_copy=0,
...
def clean_change(
    ...
    try:
        sched_course = ScheduledCourse.objects.get(course=course, work_copy=work_copy)
    except ObjectDoesNotExist:
        scheduled_before = False
        sched_course = ScheduledCourse(course=course, work_copy=work_copy)
    ...
    ret = {"course": course, "sched": sched_course, "log": course_log}

    # Rooms
    try:
        new_room = (
            Room.objects.get(name=change["room"])
    ...
    if not scheduled_before or not (
        change["start"] == sched_course.start_time and change["day"] == sched_course.day
    ):
        renumber = True
...
def room_preferences_changes_per_tutor(req, tutor, **kwargs):
    ...
    if tutor != req.user.username and not req.user.has_department_perm(
        req.department, admin=True
    ):
        bad_response["more"] = (
            "Vous ne pouvez pas changez les préférences "
    ...
    pref_list = {
        roomtypes.get(id=rt_id): [
            {"rg": Room.objects.get(id=rg_id), "rank": val}
            for rg_id, val in rg_val_dict.items()
            if val != 0
        ]
    ...
    for rt in pref_list:
        ...
        for pref in pref_list[rt]:
            ...
            for rg in prev_rg:
                bulk.append(
                    RoomSort(tutor=tutor, for_type=rt, prefer=rg, unprefer=pref["rg"])
                )

    RoomSort.objects.filter(
        for_type__department=req.department, tutor=tutor
    ).select_related("for_type").delete()
...
def preferences_changes(req, year, week, helper_pref):
    ...
    if not helper_pref.filter().filter(week=week).exists():
        for pref in helper_pref.filter().filter(week=None):
            new_dispo = helper_pref.generate(
                week, pref.day, pref.start_time, pref.duration, pref.value
            )
    ...
    for a in changes:
        ...
        for pref in a["val_inter"]:
            new_dispo = helper_pref.generate(
                week, a["day"], pref["start_time"], pref["duration"], pref["value"]
            )
            logger.info(new_dispo)
            new_dispo.save()
...
def user_preferences_changes(req, year, week, username, **kwargs):
    ...
    try:
        ...
        response["more"] = "Problème d'utilisateur."
        return JsonResponse(response)
...
def room_preferences_changes(req, year, week, room, **kwargs):
    ...
    if (
        len(set(req.user.departments.all()).intersection(set(room.departments.all())))
        == 0
    ):
        response["more"] = "Non autorisé, réclamez plus de droits."
        return JsonResponse(response)
...
def decale_changes(req, **kwargs):
    ...
    for c in change_list:
        ...
        with transaction.atomic():
            ...
            if c["d"] != "" and c["t"] != -1:
                scheduled_course = ScheduledCourse.objects.get(
                    course=changing_course, work_copy=0
                )
                cache.delete(
                    get_key_course_pl(
                        req.department.abbrev, old_week, scheduled_course.work_copy
                    )
                )
                ...
                cache.delete(get_key_course_pp(req.department.abbrev, old_week, 0))
                # note: add work_copy in Cours might be of interest

            pm = CourseModification(
                course=changing_course,
                old_week=old_week,
...
def contact(req, tutor=None, **kwargs):
    ...
    if req.method == "POST":
        ...
        if form.is_valid():
            ...
            recip_send = [
                Tutor.objects.get(username=dat.get("recipient")).email,
                dat.get("sender"),
            ]
            try:
                email = EmailMessage(
        ...
        form = ContactForm(initial={"sender": init_mail})

    if tutor is not None:
        tutor_abbrev = Tutor.objects.get(username=tutor)
        form = ContactForm(initial={"recipient": tutor_abbrev})

    return TemplateResponse(req, "base/contact.html", {"form": form, "ack": ack})
...
def send_email_proposal(req, **kwargs):
    ...
    try:
        for change in recv_changes:
            new_courses = clean_change(
                week, 0, change, work_copy=work_copy, initiator=initiator, apply=False
            )
            same, changed = new_courses["log"].strs_course_changes(
                course=new_courses["course"], sched_course=new_courses["sched"]
            )
            msg += same + changed + "\n"
            impacted_inst.add(new_courses["course"].tutor)
            impacted_inst.add(new_courses["sched"].tutor)
...
def visio_preference(req, tutor=None, id=None, **kwargs):
    ...
    if req.method == "POST":
        ...
        try:
            ...
            pref, created = UserPreferredLinks.objects.get_or_create(user=user)
...
def module_description(req, module=None, **kwargs):
    ...
    return TemplateResponse(
        req,
        "base/module_description.html",
        {"form": form, "req": req, "module": module},
    )
