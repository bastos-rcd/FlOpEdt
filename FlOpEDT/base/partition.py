...
class Partition(object):
    ...
    def add_week_end(self, days_of_week):
        ...
        i = 0
        while weekend_indexes[len(weekend_indexes) - 1] - weekend_indexes[0] >= len(
            weekend_indexes
        ) and i < len(weekend_indexes):
            weekend_indexes.append(weekend_indexes[0] + 7)
            weekend_indexes.remove(weekend_indexes[0])
            i += 1
        if weekend_indexes[len(weekend_indexes) - 1] - weekend_indexes[0] >= len(
            weekend_indexes
        ):
            return False
    ...
    def add_night_time(self, day_start_time=None, day_end_time=None):
        ...
        if self.intervals[0][0].start.hour < start_hours or (
            self.intervals[0][0].start.hour == start_hours
    ...
    def find_all_available_timeinterval_with_key(self, key, duration=None):
        ...
        while i < len(self.intervals):
            if (
                key in self.intervals[i][1]
                and self.intervals[i][1]["available"]
                and not self.intervals[i][1]["forbidden"]
            ):
                current_duration = 0
                start = self.intervals[i][0].start
                while (
                    i < len(self.intervals)
                    and key in self.intervals[i][1]
                    and self.intervals[i][1]["available"]
                    and not self.intervals[i][1]["forbidden"]
                ):
    ...
    def add_slot(self, interval, data_type, data):
        ...
        if (
            interval.start >= self.intervals[len(self.intervals) - 1][0].end
        ...
        while i < len(self.intervals) and interval.end > self.intervals[i][0].start:
            ...
            # IF WE ARE INSIDE AN EXISTING INTERVAL
                ...
                self.intervals[i][0].start <= interval.start
                and self.intervals[i][0].end >= interval.end
            ):
                new_part = 1
                if self.intervals[i][0].end != interval.end:
                    self.intervals.insert(
                        i + 1,
                        (
                            TimeInterval(interval.end, self.intervals[i][0].end),
    ...
    def add_data(self, data_type, data, interval_index):
        ...
        if "available" in data:
            self.intervals[interval_index][1]["available"] = (
                self.intervals[interval_index][1]["available"] or data["available"]
            )
        if "forbidden" in data:
            self.intervals[interval_index][1]["forbidden"] = (
                self.intervals[interval_index][1]["forbidden"] or data["forbidden"]
            )
        if (
            not data_type in self.intervals[interval_index][1]
            and data_type != "all"
        ...
        if data_type == "user_preference":
            ...
            if "period" in self.intervals[interval_index][1][data_type]:
                ...
                self.intervals[interval_index][1][data_type]["period"] = data[
                    data_type
                ]["period"]
            if "tutors" in self.intervals[interval_index][1][data_type]:
                ...
                self.intervals[interval_index][1][data_type]["tutors"] = data[
                    data_type
                ]["tutors"]
            ...
            if "tutor_status" in self.intervals[interval_index][1][data_type]:
                ...
                self.intervals[interval_index][1][data_type]["tutor_status"] = data[
                    data_type
                ]["tutor_status"]
    ...
    def get_available_partition_for_course(course, week, department):
        ...
        if course.tutor is not None:
            ...
            possible_tutors_1 = set(
                ModulePossibleTutors.objects.get(
