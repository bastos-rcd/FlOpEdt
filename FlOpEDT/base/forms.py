...
class ContactForm(forms.Form):
    sender = forms.EmailField(label=_("Your e-mail address :"), required=True)
    recipient = forms.CharField(
        label=_("Recipient :"), max_length=20, help_text=_("Pseudo")
    )
    ...
    message = forms.CharField(
        label=_("Body of the message :"), max_length=2000, widget=forms.Textarea()
    )
...
class PerfectDayForm(forms.Form):
    def __init__(self, *args, **kwargs):
        ...
        self.fields["max_time_per_day"] = forms.IntegerField(
            label=_("Maximum"), min_value=1, max_value=9, required=False, initial=6
        )
