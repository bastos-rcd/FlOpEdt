        ...
        if self.dept is None:
            if self.tutor is not None:
                for d in self.tutor.departments.all():
                    uds = pm.UserDepartmentSettings.objects.get(
                        department=d, user=self.tutor
                    )
                    if uds.is_main:
...
@method_decorator(
    name="list",
    decorator=swagger_auto_schema(
        manual_parameters=[week_param(), year_param(), dept_param(), work_copy_param()]
    ),
)
...
class CourseTypeDefaultWeekViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        # Filtering
        if department is not None:
            qs = qs.select_related("train_prog__department").filter(
                train_prog__department__abbrev=department
            )
...
class TutorCoursesFilterSet(filters.FilterSet):
    tutor_name = filters.CharFilter(
        field_name="user__departments__train_prog__module__module__tutor__username",
        required=True,
    )
...
class ExtraSchedCoursesViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        return (
            bm.ScheduledCourse.objects.filter(
                tutor__username=user,
...
@method_decorator(
    name="list",
    decorator=swagger_auto_schema(manual_parameters=[dept_param(required=True)]),
)
...
@method_decorator(
    name="list",
    decorator=swagger_auto_schema(manual_parameters=[dept_param(required=False)]),
)
