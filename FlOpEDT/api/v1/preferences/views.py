...
from api.shared.params import week_param, year_param, user_param, dept_param, \
    from_date_param, to_date_param, date_param, weekday_param
...
class UserPreferenceViewSet(viewsets.ModelViewSet):
    ...
    def set_default_params(self):
        wanted_date = self.request.query_params.get('date', None)
        if wanted_date is not None:
            weekday = wanted_date.weekday()
        else:
            weekday = self.request.query_params.get('weekday', None)

        self.params["weekday"] = weekday
        self.unset_singular_params()

    def set_singular_params(self):
        self.params['start_time__date'] = self.request.query_params.get('date')

    def unset_singular_params(self):
        self.params.pop('start_time__date', None)

    def unset_default_params(self):
        self.params.pop('weekday', None)
...
@method_decorator(
    ...
    decorator=swagger_auto_schema(
        manual_parameters=[weekday_param(),
                           user_param(),
                           dept_param()],
        operation_description="Default user preferences",
    )
)
class UserPreferenceDefaultDayViewSet(UserPreferenceViewSet):
    permission_classes = [IsAdminOrReadOnly]
...
@method_decorator(
    name='list',
    decorator=swagger_auto_schema(
        manual_parameters=[date_param(required=True),
                           user_param(),
                           dept_param(),
...
class DatedUserPreferenceViewSet(UserPreferenceViewSet):
    '''
    Helper for user preferences:
    - read parameters
    - build queryset
