<template>
  <div>
    ...
    >
      ...
      <template #body>
        <span>
          You are about to remove a repetition. Doing so will cause the next reservations to be lost forever and unlink
          all previous reservations from this repetition.</span
        >
        <br />
    ...
    <ModalDialog :is-open="isReservationEditDialogOpen" :is-locked="isDialogLocked">
      ...
      <template #body>
        <span>
          You are about to save changes to a periodic reservation. Would you like to apply the changes to the future
          reservations too?</span
        >
        <br />
        <span><b>Applying the changes will overwrite previous values.</b></span>
      </template>
      <template #buttons>
        <button type="button" class="btn btn-secondary" @click.stop="onReservationEditRefuse">Only this one</button>
    ...
    <ModalDialog :is-open="isReservationConflictDialogOpen" :is-locked="isDialogLocked">
      ...
      <template #buttons>
        <div v-if="current_conflict && current_conflict.possible_reservations.length > 0">
          <button type="button" class="btn btn-secondary" @click.stop="onReservationConflictRefuseCreation">No</button>
          <button type="button" class="btn btn-primary" @click.stop="onReservationConflictAcceptCreation">Yes</button>
        </div>
        <div v-else>
      ...
      <template #body>
          ...
          >
            <template #input="{ value }">
              <div class="form-floating">
                <input :id="generateId('date')" type="text" class="form-control" placeholder="Date" :value="value" />
        ...
        <div v-else class="mb-3 border rounded p-2">
          <div class="mb-2 text-center">
            <button type="button" class="btn btn-danger btn-sm" @click="removePeriodicity">Remove repetition</button>
          </div>
                ...
                <template #input="{ value }">
                  <div class="form-floating">
                    <input
                      ...
                      :disabled="isPeriodic"
                    />
                    <label :for="generateId('periodicity-end-date')" class="form-label">Until</label>
                  </div>
                </template>
              </DayPicker>
            </div>
...
import PeriodicitySelect from '@/components/periodicity/PeriodicitySelect.vue'
