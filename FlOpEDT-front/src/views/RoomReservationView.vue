      ...
      <div class="row">
        ...
        <div class="col">
          ...
          <div class="row">
            <div class="col-auto">
              <div>
                <span :style="{ color: scheduledCourseColor }">■ </span
                >{{ $t('roomreservation.schedule-caption.course') }}
              </div>
            </div>
...
import {
  isRoomSelected,
  deleteReservationPeriodicity,
  createDateId,
  getCurrentWeekDays,
  addTo,
  handleReason,
  isRoomInSelectedDepartments,
  createTime,
  listGroupBy,
  parseReason,
...
import type {
  ...
  RoomAttributes,
  RoomAttributeValues,
  TemporaryCalendarSlots,
  ScheduledCourseSlots,
  RoomReservationSlots,
} from '@/ts/reservationDataTypes'
    ...
    return Object.fromEntries(
      Object.entries(scheduledCourses.perDepartment.value).map((entry) => [
        ...
        entry[1].filter((course: ScheduledCourse) => {
          ...
          if (course.room) {
            filtering_result =
              isRoomSelected(course.room.id, selectedRoom.value) &&
              isRoomInSelectedDepartments(course.room.id, selectedDepartments.value)
          }
...
const courseTypes: CourseTypes = {
  perDepartment: ref({}),
  listFilterBySelectedDepartments: computed(() => {
    return Object.values(filterBySelectedDepartments(courseTypes.perDepartment.value, selectedDepartments.value)).flat(
      1
    )
  }),
}
    ...
    const out: { [day: string]: Array<CalendarSlot> } = Object.fromEntries(
      Object.entries(roomReservationSlots.perDay.value).map((entry) => [
        ...
        entry[1].filter((slot) => {
          ...
          return (
            isRoomSelected(room, selectedRoom.value) && isRoomInSelectedDepartments(room, selectedDepartments.value)
          )
        }),
      ])
    )
    ...
    const out: { [day: string]: { [roomId: string]: Array<CalendarSlot> } } = Object.fromEntries(
      Object.entries(roomReservationSlots.perDay.value).map((entry) => [
        ...
        listGroupBy(
          ...
          entry[1].filter((slot) =>
            isRoomInSelectedDepartments(
              (slot.slotData as CalendarRoomReservationSlotData).reservation.room,
...
watch(
  ...
  async (newDate, oldDate) => {
    ...
    scheduledCourseStore
      .fetchScheduledCourses(newDate)
      .then(() => updateScheduledCourses(departmentStore.getAllDepartmentsFetched))
...
function createRoomReservationSlot(reservation: RoomReservation): CalendarSlot {
  ...
  const slotData: CalendarRoomReservationSlotData = {
    ...
    onPeriodicityDelete: () => {
      ...
      return reservationsRemovalPromise.then((_: any) =>
        deleteReservationPeriodicity(periodicityId)
