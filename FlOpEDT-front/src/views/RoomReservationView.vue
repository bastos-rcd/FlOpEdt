...
function handleDepartmentNameClick(deptId: number) {
    selectedDepartment.value = departmentStore.getAllDepartmentsFetched.find((dept: Department) => dept.id === deptId)
}
...
function getScheduledCourseDepartment(course: ScheduledCourse): Department | undefined {
    ...
    if (departmentEntry) {
        ...
        return departmentStore.getAllDepartmentsFetched.find((dept: Department) => `${dept.id}` === deptId)
    }
    return undefined
}
